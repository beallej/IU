  ...
  (lambda (exp env) 
    ...
    (pmatch exp         
            ...
            (`(cast ,L ,e : ,T1 -> ,T2)        
             (cset '1)
             (pmatch `(,T1 ,T2)
                     (`(,T3 ,T4)(guard (equal? T1 T2)) ;(display "\n") (display `("T3 T3:   " ,T1 "\t" ,T2)) (display "\n")  
                                (evalRec e env))
                     (`((-> ,T11 ,T12) (-> ,T21 ,T22))
                      ;(display "\n") (display `("(-> ,T11 ,T12) (-> ,T21 ,T22):   " ,T1 "\t" ,T2))(display "\n")
                      (let ((newval (gensym)))
                        (cset 'e)
                        (set! type-obs (extend-Trec newval T21 type-obs))
                        `(closure ,newval ,T21 (cast ,L (call ,e (cast ,L ,newval : ,T21 -> ,T11)) : ,T12 -> ,T22) ,T22 ,env)))                     
                     (`(,T3 dyn)
                      ;(display "\n") (display `("T3 dyn:   " ,T1 "\t" ,T2))(display "\n")
                      `(cast ,L ,(evalRec e env) : ,T3 -> dyn))
                     (`(dyn ,T4)
                      ;(display "\n") (display `("dyn T4:   " ,T1 "\t" ,T2))(display "\n")
                       ...
                       (`(closure ,x1 ,par-T ,e1 ,ret-T ,env1)                        
                        (pmatch `(,T1 ,T2)
                                (`((-> ,T11 ,T12) (-> ,T21 ,T22))                                 
                                 (let ((newval (gensym)))                                   
                                   (set! type-obs (extend-Trec newval T21 type-obs))
                                   `(,x (closure ,newval ,T21 (cast ,L (call val (cast ,L ,newval : ,T21 -> ,T11)) : ,T12 -> ,T22) ,T22 ,env1) record)))
                                (`(,T3 dyn) `(,x (cast ,L ,val : ,T3 -> dyn) record))))
                       (`(cast ,M ,f : ,T1a -> dyn)
                        (if (consistent? T1a T2)
                            ;(begin (display "\nF:\t") (display f) (display "\n") `(,x ,f record))
                            `(,x ,f record)
                                     (error "cast between inconsistent types! blame" L "and" M)))
