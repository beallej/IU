  ...
  (lambda (exp)
    (let ((typed-e (typecheck '() exp)))  
      (begin
        ...
        (display "\n")(display "\n")
        (display "\n\nOriginal check: \n")
        (display typed-e);(display "\n")(display "\n")
        (display "\n\nEvaluation: \n")
        ;(display "\n")
        (display (evalRec typed-e '())) ;(display "\n")(display "\n") ;(display (evalRec typed-e '()))
        (display "\n\nCoverage: \n")
        (display (cov-pc))
        (display "%")
        (display "\n\nType Observations: \n")
             ...
             (pmatch `(,T1 ,T2)
                     ...
                     (`((-> ,T11 ,T12) (-> ,T21 ,T22))
                      (let ((newval (gensym)))
                        ...
                        `(closure ,newval (cast ,L (call ,e (cast ,L ,newval : ,T21 -> ,T11)) : ,T12 -> ,T22) ,env)))                     
                     (`(,T3 dyn)
                 ...
                 (pmatch v1
                         (`(,x1 ,ans record)
                          (pmatch ans
                                  (`(closure ,x ,e11 ,env11)                                                             
                                   (cset '1)
                                   (let ((evaled (evalRec e11 (extend-env x v2 env11))) (tv2 (type v2)))
                                     (set! type-obs (extend-Trec x1 `(-> ,tv2 ,(type evaled)) type-obs))
                                     (set! type-obs (extend-Trec x tv2 type-obs))
                                     evaled))
                                  (`(cast ,L (closure ,x ,e11 ,env11) : ,T1 -> ,T2)                                   
                                   (pmatch (meet T1 T2 L)
                                           (`(-> ,T3 ,T4)
                                            (display `("\n" ,T1 ,T2 "\n"))
                                            (let* ((tv2 (type v2)) (newtype (meet T3 tv2 L)))
                                              (set! type-obs (extend-Trec x newtype type-obs))
                                              (cset '1) 
                                              (let ((evaled (evalRec e11 (extend-env x v2 env11))))
                                                (set! type-obs (extend-Trec x1 `(-> ,newtype ,(meet (type evaled) T4 L)) type-obs))
                                                evaled)))))))
                         (`(closure ,x ,e11 ,env11)                         
                         ...
                         (`(cast ,L (closure ,x ,e11 ,env11) : ,T1 -> ,T2)
                          (pmatch (meet T1 T2)
                                  (`(-> ,T3 ,T4)
                                   (set! type-obs (extend-Trec x (meet T3 (type v2) L) type-obs))
                                   (display `("\n" ,T1 ,T2))
                                   (cset '1)                                   
                                   (evalRec `(,e11 ,T4) (extend-env x v2 env11)))))
...
(check-error (evals (unique '((lambda (y : dyn) (((lambda (x : int) (#t : dyn A))  (y : int B) C) : int D)) 5 E))))
(check-error (evals (unique '(((lambda (x : int) (#t : dyn ML)) : (-> int int) MO) 7 B))))
(check-error (evals (unique '(((lambda (x : int) (#t : dyn ML)) : (-> int int) MO) 7 B))))
(check-error (evals (unique '((#t : dyn  L) : int M))))
(check-error (evals (unique '((((lambda (x : int) (#t : dyn K)) : dyn L) : (-> int int) P) 42 N))))
(check-error (evals (unique '(((lambda (x : int) ((lambda (y : int) 42) : (-> dyn dyn) L)) 42 M) (#t : dyn O) N))))
(check-error (evals (unique '((lambda (x) (x 3 L)) ((lambda (y : dyn) (inc y O)) : (-> bool int) M) N))))
(check-error (evals (unique '((lambda (z) (z 3 M)) ((lambda (x : int) (#t : dyn L)) : (-> int int) O) N))))
(check-error (evals (unique '(((((lambda (x : int) x) : (-> int dyn) L) : (-> dyn dyn) M) : (-> dyn bool) N) 7 O))))
(check-error (evals (unique '(((lambda (x : int) (#t : dyn L)) : (-> int int) M) 7 N))))
(check-error (evals (unique '((lambda (x : int) #t) : (-> int int) L))))
(check-error (evals (unique '(((lambda (x : int) #t) : (-> int dyn) L) : (-> int int) error_on_collapse)))) ;should error in collapse
(check-error (evals (unique '(((lambda (x : int) 
                                 (((((#t : dyn L) : bool M) : dyn N) : bool O) : dyn P))
                               : (-> int int) Q) 42 R))))
(check-error (evals (unique '((((lambda (x : int) #t) : (-> int dyn) L) : (-> int int) M) 42 N) )))
(test)
